...
module.exports = async (req, res) => {
  ...
  try {
    // Cek cache dulu untuk mengurangi request scraping
    const cacheKey = 'rizstore_catalog';
    const cachedData = await kv.get(cacheKey);
    
    // Jika ada cache valid (tidak lebih dari 30 menit), gunakan cache
    if (cachedData && cachedData.timestamp > Date.now() - 1800000) { // 30 menit
      console.log('Returning cached data from Vercel KV');
    ...
    // Jika tidak ada cache atau cache expired, scrape data baru
    console.log('Cache miss or expired, scraping new data');
    ...
    await kv.set(cacheKey, {
      ...
      timestamp: timestamp
    }, { ex: 1800 }); // expired dalam 30 menit
    ...
    // Jika terjadi error, coba ambil dari cache (jika ada)
    try {
      const cacheKey = 'rizstore_catalog';
      const cachedData = await kv.get(cacheKey);
    ...
    // Jika tidak ada cache atau gagal mengambil cache, gunakan fallback statis
